...
static void LISaddMission(struct WADDIR  *dir, Int16 found)
{
	Int16 l=0,dummy=0;
	/*check*/
	if(LISlmp.Pos>=LISlmp.Top) printf("BUG: LisTsm"); /*level list too small*/
...
{   /*check current position*/
	 if((L->Pos)>=L->Top) printf("BUG: LisSml"); /*list count too small*/
	 /* ADD in List */
...
{ /* SUBSTIT in List */
  ...
  printf("WARNING: entry %s might be ignored by DOOM.", lump_name (dir[0].name));
  LISadd(L,dir);
}
/* Move In List
	...
	{ type= iiden[i] ;
	  switch(type&EMASK)
	  {
		 ...
		 case EZZZZ:
			break;
		 default:
			printf("BUG: LisUkI");/*unknown entry type in IWAD*/
			break;
	  }
	}
	free(iiden);
	...
	{ type= piden[p] ;
	  ...
	  {   /*special treatment for missions*/
       ...
       case EZZZZ:
         break;
       default:
         printf("BUG: LisUkP");/*unknown entry type in PWAD*/
        break;
      }
   }
   free(piden);
