...
{ FILE *exe;
  ...
  if(exe==NULL)printf("ERROR: Can't open %s",doomexe);
  if(fseek(exe,posit,SEEK_SET))printf("ERROR: Can't seek in %s",doomexe);
  ...
  { if(remains==0)
    ...
    { case 1:
      ...
      case 2:
       ...
       { lastpos=posit;
	 printf("PHASE: Current offset:0x%-6lx\tfound %ld strings\n",posit,found);
       }
      default:
       count=0;
       istr=0;
...
{ FILE *exe;
  ...
  if(exe==NULL)
         printf("ERROR: Can't open %s for writing",doomexe);
  ...
  while(1)
  {
    ...
    if(strln<thres)   printf("ERROR: String too small");
    for(istr=anewln;istr<strln;istr++) anew[istr]=' ';
    anew[strln]=str[strln]='\0';
#ifdef DEBUG
    printf("DETAIL: substit\t#%s# \nby\t#%s#\n",str,anew);
#endif
    lastpos = posit = start;
    if(fseek(exe,posit,SEEK_SET))
       printf("ERROR: Can't seek in %s",doomexe);
    found=-1;
    ...
    { if(remains==0){ remains=fread(raw,1,RAWSIZE,exe); iraw=0;}
	 ...
	 if(istr==strln) {same=TRUE;ref=-1;}
      ...
      { if(same==TRUE)
        { if((c=='\0')||(c=='\n'))
	  { printf("DETAIL: Found at\t0x%-6lx\t#%s#\n",lastpos,str);
	    if(found>=0)
	    { printf("WARNING: String defined twice: #%s#",str);
	      found=0;break;
	    }
	    found=lastpos;
	  }
    ...
    if(found>0)
    { printf("DETAIL: Writing at\t0x%-6lx\t#%s#\n",found,anew);
      if(fseek(exe,found,SEEK_SET)) printf("ERROR: Can't seek %s",doomexe);
      if(fwrite(anew,1,strln,exe)!=strln) printf("ERROR: Can't write %s correctly",doomexe);
    }
    else
    printf("WARNING: Can't insert #%s#",anew);
  }
  fclose(exe);
  free(raw);
  free(str);
