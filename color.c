...
{  Int16 i,test,min=0x7FFF;
   UInt8 idxmin='\0';
   if(COLok!=TRUE) printf("BUG: COLok\n");
   for(i=0;i<256;i++)
...
{ Int16 count,idx,nextidx;
  ...
  printf("BUG: Can't hash Doom pal!\n");
}
...
{  Int16 i;
   ...
   /*Int16 R,G,B;*/
   if(COLok!=FALSE) printf("BUG: COLok\n");
   if((UInt16)Colsz< 256*sizeof(struct PIXEL)) printf("BUG: Color entry too small\n");
   COLok=TRUE;
   ...
   */
   ...
   printf("INFO: Color palette is %s\n", name ? name : "unknown");
   if (name == NULL)
       printf("WARNING unknown palette. Using colour 0xff as transparent"
...
struct PIXEL *COLaltPalet(void)
{
  ...
  if (titlepal_data == NULL)
  {
    ...
    printf("WARNING: TITLEPAL not found. Using PLAYPAL instead.\n");
  ...
  else
  {
    ...
    if (titlepal_size < 3 * NCOLOURS)
      printf("WARNING: TITLEPAL too short (%ld). Filling with black.",
	  (long) titlepal_size);
    COLpalAlt = (struct PIXEL *) malloc(NCOLOURS * sizeof *COLpalAlt);
...
/*unused!*/   UInt8 r,g,b;
/*unused!*/
/*unused!*/   if(COLok!=FALSE) printf("BUG: COLok");
/*unused!*/   if(Colsz< 256*sizeof(struct PIXEL)) printf("BUG: Color entry too small");
/*unused!*/   COLok=TRUE;
...
/*unused!*/   }
/*unused!*/   if(COLinvisib!=(UInt8)0xF7)printf("WARNING: Strange PLAYPAL invisible color");
/*unused!*/   if(COLinvisib==0)printf("ERROR: PLAYPAL is not correct");
	      /*
	      ** correction to doom palette
	      */
...
/*unused!*/{ if(COLok!=TRUE) printf("BUG: COLok");
