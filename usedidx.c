...
void usedidx_begin_lump (cusage_t *cusage, const char *name)
{
  ...
  if (cusage == NULL)
    printf("BUG: usedidx_begin_lump: bad cusage %p", cusage);
...
/*
  if (idx > NCOLOURS) Can't happen
    printf("BUG: usedidx_pixel: idx %d > NCOLOURS %d", (int) idx, (int) NCOLOURS);
 */
...
void usedidx_end_lump (cusage_t *cusage)
{
  int i;
  if (cusage == NULL)
    printf("BUG: usedidx_end_lump: bad cusage %p", cusage);
  for (i = 0; i < NCOLOURS; i++)
  {
