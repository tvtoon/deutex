...
{  Bool res;
   ...
   { case PICGIF: extens="GIF";break;
     ...
     default: printf("BUG: img type");
   }
   res = MakeFileName(file,DataDir,dir,"",name,extens);
   if((WSafe==TRUE)&&(res==TRUE))
   { printf("WARNING: Will not overwrite file %s",file);
     return TRUE;
   }
   ...
   res = PICsaveInFile(file,type,buffer,size,pinsrX,pinsrY,Picture, name,
       cusage);
   if(res==TRUE)printf("DETAIL: Saved picture as %s\n",file);
   free(buffer);
   return res;
}
...
{ static struct WADINFO pwad;
  ...
  printf("PHASE: Extracting entries from WAD %s\n",wadin);
  ...
  */
  ...
  /* Read PLAYPAL */
  {
	...
	printf("ERROR: Can't find PLAYPAL.");
    }
  ...
  */
  ...
  { pnm=WADRfindEntry(&iwad,"PNAMES");
    if(pnm>=0)
      Pnam=WADRreadEntry(&iwad,pnm,&Pnamsz);
    else printf("ERROR: Can't find PNAMES in main WAD");
  }
  PNMinit(Pnam,Pnamsz);
  free(Pnam);
  /*
  ...
  */
  ...
  else
  {
     ...
     {
       char comment[81];
       sprintf (comment, "DeuTex %.32s by Olivier Montanuy", _VERSION_);
  ...
  */
  if(select&BLEVEL)
  { printf("PHASE: Extracting levels...\n");
...
{ EntryFound=FALSE;
  ...
  */
  if(select&BSOUND)
  { printf("PHASE: Extracting sounds...\n");
    ostart=0x80000000L;osize=0;
    for(EntryFound=FALSE,p=0;p<pnb;p++)
        ...
        { ostart=pwad.dir[p].start; osize=pwad.dir[p].size;
          ...
          { case ESNDPC:
              ...
              { buffer=(char  *)malloc(pdir[p].size);
                ...
                free(buffer);
                printf("DETAIL: Saved PC Sound as   %s\n",file);
              }
		  ...
		  FALSE);
		    ...
		    pdir[p].name);
                printf("DETAIL: Saved Sound as   %s\n",file);
                free(buffer);
              }
              TXTaddEntry(TXT,pdir[p].name,NULL,INVALIDINT,INVALIDINT,FALSE,
		  FALSE);
  ...
  */
  if(select&BMUSIC)
  { printf("PHASE: Extracting musics...\n");
    ostart=0x80000000L;osize=0;
    for(EntryFound=FALSE,p=0;p<pnb;p++)
    { if((piden[p]&EMASK)==EMUSIC)
      { if(EntryFound!=TRUE)
        ...
        { ostart=pwad.dir[p].start; osize=pwad.dir[p].size;
          ...
          { WADRsaveEntry(&pwad,p,file);
            printf("DETAIL: Saved Music as %s\n",file);
          }
          TXTaddEntry(TXT,pdir[p].name,NULL,INVALIDINT,INVALIDINT,FALSE,FALSE);
        }
      }
  ...
  */
  if(select&BGRAPHIC)
  { printf("PHASE: Extracting graphics...\n");
    ostart=0x80000000L;osize=0;
    for(EntryFound=FALSE,p=0;p<pnb;p++)
  ...
  */
  if(select&BSPRITE)
  { printf("PHASE: Extracting sprites...\n");
    ostart=0x80000000L;osize=0;
    for(EntryFound=FALSE,p=0;p<pnb;p++)
    { if((piden[p]&EMASK)==ESPRITE)
      { if(EntryFound!=TRUE)
        ...
        { ostart=pwad.dir[p].start; osize=pwad.dir[p].size;
	      ...
	      Picture,WSafe, cusage)!=TRUE)
          { printf("WARNING: failed to write sprite %s", lump_name (pwad.dir[p].name));
          }
          else
          { TXTaddEntry(TXT,pdir[p].name,NULL,insrX,insrY,FALSE,TRUE);
          }
  ...
  */
  if(select&BPATCH)
  { printf("PHASE: Extracting patches...\n");
    for(EntryFound=FALSE,p=0;p<pnb;p++)
     { if((piden[p] & EMASK)==EPATCH)
	   ...
	   if (cusage == NULL)
         ...
         { printf("WARNING: failed to write patch %s", lump_name (pwad.dir[p].name));
         }
       }
     }
  }
      ...
      { if(EntryFound!=TRUE)
        ...
        { ostart=pwad.dir[p].start; osize=pwad.dir[p].size;
          ...
          { if(strncmp(pwad.dir[p].name,"F_SKY1",6)!=0)
            printf("WARNING: failed to write flat %s", lump_name (pwad.dir[p].name));
          }
          else
            TXTaddEntry(TXT,pdir[p].name,NULL,INVALIDINT,INVALIDINT,FALSE,FALSE);
        }
  ...
  if (select & BSNEAP)
  {
	...
	{ ostart=pwad.dir[p].start; osize=pwad.dir[p].size;
	  if(XTRbmpSave(&insrX,&insrY,&pdir[p],entry_type_pictype (type),
	      ...
	      cusage) != TRUE)
	  {
	    printf("WARNING: failed to write %.20s %s",
  ...
  */
  ...
  TXTcloseW(TXT);
  printf("PHASE: End of extraction.\n");
}
...
{ static struct WADINFO pwad;
  ...
  else  printf("ERROR: Can't find PLAYPAL in main WAD");
  WADRclose(&iwad);
  pwad.ok=0;
  WADRopenR(&pwad,wadin);
  ...
  if(e<0) printf("ERROR: Can't find entry %s in WAD", lump_name (Name));
  printf("PHASE: Extracting entry %s from WAD %s\n", lump_name (entry), wadin);
  ...
  if(Found!=TRUE)
    ...
    { switch(Picture)
	  ...
	  NULL) ==TRUE)
      { printf("INFO: Picture insertion point is (%d,%d)",insrX,insrY);
        Found=TRUE;
      }
