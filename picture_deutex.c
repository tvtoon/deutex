...
{  Int16 x,y;
  ...
  */
  ...
  { write_i32_le (ColOfs + x, colnpos);
	...
	if(y >= 509)
	{ printf("WARNING: Column has more than 509 pixels."
	    " Truncating at (%d,%d).\n", (int) x, (int) y);
	  break;
	}
...
*/
static char  *PICtoRAW(Int16 *prawX,Int16 *prawY,Int16 *pXinsr,Int16 *pYinsr, const char  *pic,Int32 picsz,char transparent, const char *name, cusage_t *cusage)
{  Int16 x,y;         /*pixels*/
   const void *offsets;
...
//   int nw = 5;
   ...
   {
      char message_buf[81];
      if (parse_pic_header ((const unsigned char *)pic, picsz, &h, message_buf))
      {
	 printf("WARNING: Picture %s: %s. Skipping picture.",
   ...
   for (x = 0; x < h.width; x++)
   {
	 ...
	 post = (const unsigned char *) (pic + ofs);
      ...
      {
	 printf("BUG: Bad colofs_size %d", (int) h.colofs_size);  /* Can't happen */
      }
      if (post < (const unsigned char *) pic)
      {
printf( "Picture %s(%d): column has bad offset %ld. Skipping column.",
        lump_name (name), (int) x, (long) ((const char *) pic - (const char *) post));
 continue;
      }
      if (post < h.data)
      {
printf( "Picture %s(%d): column has suspicious offset %ld.",
        lump_name (name),
        (int) x,
	...
	if (post > pic_end)
	{
printf( "Picture %s(%d): post starts past EOL. Skipping rest of column.",
	...
	y           = *post++;
	post_length = *post++;
	if (h.dummy_bytes) post++;
	post_end = post + post_length;
        if (post_end > pic_end)
	{
printf( "Picture %s(%d): post spans EOL. Skipping rest of column.",
	      lump_name (name), (int) x);
/* Used to be fatal--AYM 1999-10-16 */
  break;
	...
	/* Read the middle of the post */
        ...
        { if (y >= h.height)
	  {
printf( "Picture %s(%d): post Y-offset too high. Skipping rest of column.",
		lump_name (name), (int) x);
/* Used to be fatal--AYM 1999-10-16 */
	    break;
	  }
   ...
   /*return*/
   printf( "Picture %s: ", lump_name (name));
   if (cusage != NULL)
...
/******************* Snea module ************************/
static char *snea_to_raw (Int16 *prawX, Int16 *prawY,/* Int16 *pXinsr, Int16 *pYinsr,*/ const char *sneabuf, Int32 sneasz, const char *name, cusage_t *cusage)
{
  ...
  if (pixels + 2 != (unsigned int)sneasz)
  {
    printf("WARNING: Snea %s: mismatch between size and geometry. Skipping snea.",
	lump_name (name));
    free(raw);
    return NULL;
  }
  ...
  if (cusage != NULL)
    usedidx_rectangle ((char *)raw, pixels, name, cusage);
...
static char *BMPtoRAW(Int16 *prawX,Int16 *prawY,char *file)
{
   ...
   */
   ...
   if(fd==NULL)
      printf("ERROR: Can't open %s for reading",file);
   if(fread(sig,2,1,fd)!=1)
      printf("ERROR: Can't read sig of BMP %s",file);
   if(strncmp(sig,"BM",2)!=0)
      printf("ERROR: Bmp: signature incorrect");

   head=(struct BMPHEAD  *)malloc(sizeof(struct BMPHEAD));
   if(fread(head,sizeof(struct BMPHEAD),1,fd)!=1)
     printf("ERROR: Bmp: can't read header");
   /*
   ** check the BMP header
   */
   if (peek_i32_le (&head->compress) != 0) printf("ERROR: Bmp: not RGB");
   read_i32_le (&head->startpix, &startpix);
   szx = (Int16) peek_i32_le (&head->szx);
   szy = (Int16) peek_i32_le (&head->szy);
   if(szx<1) printf("ERROR: Bmp: bad width");
   if(szy<1) printf("ERROR: Bmp: bad height");
   ...
   */
   ...
   { case 24:
	 ...
	 if (! warned_once)
	 {
	   printf("INFO: Warning: Color quantisation is slow. Use 8 bit BMP.\n");
	   warned_once = 1;
	 }
       }
     ...
     case 8:
       ...
       if(ncols>256)                    printf("ERROR: Bmp: palette should be 256 color");
       if(ncols<=0) ncols=256;   /*Bug of PaintBrush*/
       paletsz=(ncols)*sizeof(struct BMPPALET);
       /*set position to palette  (a bit hacked)*/
       if(fseek(fd,0xEL+peek_i32_le(&head->headsz),SEEK_SET))printf("ERROR: Bmp: seek failed");
       /*load palette*/
       palet=(struct BMPPALET  *)malloc(paletsz);
       if(fread(palet,(size_t)paletsz,1,fd)!=1)   printf("ERROR: Bmp: can't read palette");
       for(p=0;p<ncols;p++)
       ...
       break;
     default:
        printf("ERROR: Bmp: type not supported (%d bits)",nbits);
   }
   ...
   if(head->pixlen<bmpxy)
     if(peek_i32_le(&head->bmplen)<(startpix+bmpxy))
       printf("ERROR: Bmp: size of pixel area incorrect");

   free(head);
   /* seek start of pixels */
   if(fseek(fd,startpix,SEEK_SET))      printf("ERROR: Bmp: seek failed");
   /* read lines */
   ...
   for(y=szy-1;y>=0;y--)
   {  if(fread(line,(size_t)linesz,1,fd)!=1)    printf("ERROR: Can't read BMP line");
      for(x=0;x<szx;x++)
      {  switch(nbits)
         {  case 24:
...
static void RAWtoBMP(char *file,char  *raw,Int16 rawX, Int16 rawY,struct PIXEL  *doompal)
{
  ...
  fd=fopen(file,FOPEN_WB);
   ...
   if(fd==NULL)
     printf("ERROR: Can't create %s (%s)", file, strerror (errno));
   ncol=256;
   ...
   strncpy(sig,"BM",2);
   if(fwrite(sig,2,1,fd)!=1)   printf("ERROR: Can't write file %s",file);
   /*Header*/
   head=(struct BMPHEAD *)malloc(sizeof(struct BMPHEAD));
   ...
   */
   ...
   { for(x=0;x<rawX;x++)
     ...
     if(fwrite(bmpidxs,1,linesz,fd)!= (UInt16)linesz)
       printf("ERROR: Can't write file %s",file);
   }
   free(bmpidxs);
   /*done*/
...
static void RAWtoPPM(char *file, char *raw, Int16 rawX, Int16 rawY, struct PIXEL *doompal)
{
   ...
   /* data */
   {
     ...
     for (p = (unsigned char *)raw; p < pmax; p++)
...
static char *PPMtoRAW(Int16 *prawX,Int16 *prawY,char *file)
{
   ...
   fd=fopen(file,FOPEN_RB);
   if(fd==NULL)     printf("ERROR: Can't open %s",file);
   if(getc(fd)!='P' || getc(fd) != '6')
   {
     fclose(fd);
     printf("ERROR: %s is not a rawbits PPM (P6) file", file);
   }
   c=getc(fd);
   ...
   for(n=0;n<10;n++){if(!isdigit(c))break;buff[n]=c;c=getc(fd);}buff[n]='\0';
   /* data */
   if(rawX<1) {fclose(fd);printf("ERROR: %s: bad width", file);}
   if(rawY<1) {fclose(fd);printf("ERROR: %s: bad height", file);}
   rawSz=((Int32)rawX)*((Int32)rawY);
   raw=(char  *)malloc(rawSz);
   if (! warned_once)
   {
     printf("INFO: Warning: Color quantisation is slow. Use ppmquant.\n");
     warned_once = 1;
   }
   for(rawpos=0;rawpos<rawSz;rawpos++)
   { if(fread(&pix,sizeof(struct PIXEL),1,fd)!=1)
     {fclose(fd);printf("ERROR: %s: unexpected EOF", file);}
...
{  UInt16 ofsx;  /*0,1 left offset*/
   UInt16 ofsy;  /*2,3 top offset*/
   UInt16 xsize; /*4,5 width*/
   UInt16 ysize; /*6,7 heigth*/
...
{  char  *raw=NULL;
   ...
   */
   ...
   if (fread(&c,1,1,fd)!=1) printf("ERROR: GIF: read error" );
   if (LWZReadByte(fd, TRUE, c) < 0) printf("ERROR: GIF: bad code in image" );
   /* read the file */
   for(rawpos=0;rawpos<rawSz;rawpos++)
   { if ((v = LWZReadByte(fd,FALSE,c)) < 0 ) printf("ERROR: GIF: too short");
     raw[rawpos]=(v&0xFF);
   }
...
*/
static Int16 GIFreadBlock(FILE *fd, char buff[256])
{ Int16 data,count,c;
...
{   Int16  Xsz=0,Ysz=0;
   ...
   */
   ...
   {
     fclose(fd);
     printf("ERROR: GIF: read error");
   }
   if (strncmp (GifIdent,"GIF87a",6) && strncmp (GifIdent,"GIF89a",6))
   {
     fclose(fd);
     printf("ERROR: GIF: not a 87a or 89a GIF");
   }
   ...
   if ((GifHead.info)&COLORMAP)
     { if(fread(GifColor,sizeof(struct PIXEL),bitPixel,fd)!= (UInt16)bitPixel)
     { fclose(fd);printf("ERROR: GIF: read error");
     }
   }
   /*
   ...
   */
   ...
   {  if(c==';') break; /* GIF terminator */
	...
	{ printf("WARNING: GIF: Other images discarded.");
	  break;
	}
	...
	Ysz = GifImage.ysize;
	if((Xsz<1)||(Ysz<1))
	{ fclose(fd);printf("ERROR: GIF: bad size");
	}
   ...
   fclose(fd);
   if(raw == NULL) printf("ERROR: GIF: No picture found");
   /*convert colors*/
   for (c=0; c<256; c++)
...
{  FILE *fd;
   ...
   fd=fopen(file,FOPEN_WB);
   if(fd==NULL)printf("ERROR: Can't open GIF file %s",file);
   rawSz = (Int32)rawX * (Int32)rawY;
   /* screen header */
...
{  char  *raw;
   ...
   printf("PHASE: GIF->RAW\n");
   MakeFileName(file,bmpdir,"","",name,"GIF");
   raw =GIFtoRAW(&rawX,&rawY,file);
   doompal = COLdoomPalet();
   printf("PHASE: RAW->BMP\n");
   MakeFileName(file,bmpdir,"","",name,"BMP");
...
{ char  *raw=NULL;
  ...
  *pYinsr=INVALIDINT; /*default insertion point Y*/
  ...
  if (cusage == NULL)
  {
	...
	break;
       default:
	printf("BUG: psvit (%d)", (int) Picture);
     }
  }
...
Int32 PICsaveInWAD(struct WADINFO *info,char *file,PICTYPE type,Int16 Xinsr,Int16 Yinsr,IMGTYPE Picture)
{
  ...
  */
  ...
  if((rawY<1)||(rawX<1))printf("ERROR: Bad picture size");
  // AYM (256 -> 509)
  if(rawY>509)printf("ERROR: Picture height can't be more than 509");
  // AYM (512 -> 1024)
  if(rawX>1024)printf("ERROR: Picture width can't be more than 1024");
  switch(type)
  { case PGRAPH: case PWEAPN: case PSPRIT: case PPATCH:
      break;
    case PFLAT: /*flats*/
      if(rawX!=64)
	printf("WARNING: %s: weird width for a flat (not 64)", fname (file));
      if(rawY!=64 && rawY!=65 && rawY != 128)
	printf("WARNING: %s: weird height for a flat (not 64, 65 or 128)", fname (file));
      break;
    case PLUMP: /*special heretic lumps*/
      if(rawX!=320)
        printf("WARNING: %s: weird width for a lump (not 320)", fname (file));
      if(rawY!=200)
        printf("WARNING: %s: weird height for a lump (not 200)", fname (file));
      break;
    default:
      printf("BUG: picwt");
  }
