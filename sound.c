...
static void SNDsaveWave(char *file,char  *buffer,Int32 size,Int32 speed)
{
  ...
  if(fp==NULL)
  { printf("ERROR: Can't open %s for writing (%s)", fname (file), strerror (errno));
  }
  /*header*/
  ...
  { sz= (size-wsize>MEMORYCACHE)? MEMORYCACHE:(size-wsize);
    if(fwrite((buffer+(wsize)),(size_t)sz,1,fp)!=1)
      printf("ERROR: %s: write error (%s)", fname (file), strerror (errno));
  }
  fclose(fp);
}
...
{ FILE *fp;
  ...
  if(fp==NULL)
    printf("ERROR: Can't open %s for reading (%s)", fname (file), strerror (errno));
  /*read RIFF HEADER*/
  if(fread(&headr,sizeof(struct RIFFHEAD),1,fp)!=1)
     printf("ERROR: %s: can't read header", fname (file));
  /*check RIFF header*/
  if(strncmp(headr.riff,"RIFF",4)!=0)
     printf("ERROR: %s: bad RIFF magic in header (%s)",
	 fname (file), short_dump (headr.riff, 4));
  if(strncmp(headr.wave,"WAVE",4)!=0)
     printf("ERROR: %s: bad WAVE magic in header (%s)",
	 fname (file), short_dump (headr.wave, 4));
  chunk=sizeof(struct RIFFHEAD);
  for(sz=0;;sz++)
  { if(sz>256)
       printf("ERROR: %s: no fmt", fname (file));
  ...
  if(peek_i16_le (&headf.tag)!=1)
    printf("ERROR: %s: not raw data", fname (file));
  if(peek_i16_le (&headf.channel)!=1)
    printf("ERROR: %s: not one channel", fname (file));
  smplrate=peek_i32_le(&headf.smplrate);
  
  for(sz=0;;sz++)
  { if(sz>256)
      printf("ERROR: %s: no data", fname (file));
    fseek(fp,chunk,SEEK_SET);
    if(fread(&headc,sizeof(struct CHUNK),1,fp)!=1)
      printf("ERROR: %s: no data", fname (file));
  ...
  if(datasize>0x100000L)
    printf("ERROR: %s: sample too long (%ld)", fname (file), (long) datasize);
  /*read data*/
  ...
  { sz = (datasize-wsize>MEMORYCACHE)? MEMORYCACHE:(datasize-wsize);
    if(fread((data+(wsize)),(size_t)sz,1,fp)!=1)
      printf("ERROR: %s: error reading data", fname (file));
  }
  fclose(fp);
  *psize=datasize;
...
{ FILE *fp;
  ...
  fp=fopen(file,FOPEN_WB);
  if(fp==NULL)
    printf("ERROR: AU: can't open %s for writing (%s)", fname (file), strerror (errno));
  /*header*/
  ...
  heada.info[0]='\0';
  if(fwrite(&heada,sizeof(struct AUHEAD),1,fp)!=1)printf("ERROR: AU: write error");
  for(i=0;i<size;i++)
  { buffer[i]-=0x80;
  }
  for(wsize=0;wsize<size;wsize+=sz)
  { sz= (size-wsize>MEMORYCACHE)? MEMORYCACHE:(size-wsize);
    if(fwrite((buffer+(wsize)),(size_t)sz,1,fp)!=1)printf("ERROR: AU: write error");
  }
    fclose(fp);
}
...
{ FILE *fp;
  ...
  fp=fopen(file,FOPEN_WB);
  if(fp==NULL)
    printf("ERROR: VOC: can't open %s for writing\n%s\n", fname(file), strerror (errno));
  /*VOC header*/
  ...
  for(wsize=0;wsize<size;wsize+=sz)
  { sz= (size-wsize>MEMORYCACHE)? MEMORYCACHE:(size-wsize);
    if(fwrite((buffer+(wsize)),(size_t)sz,1,fp)!=1)printf("ERROR: VOC: write error");
  }
...
{ FILE *fp;
  ...
  fp=fopen(file,FOPEN_RB);
  if(fp==NULL)
    printf("ERROR: VOC: can't open %s for reading (%s)", fname (file), strerror (errno));
  /*read VOC HEADER*/
  if(fread(&headv,sizeof(struct VOCHEAD),1,fp)!=1) printf("ERROR: VOC: can't read header");
  if(strncmp(VocId,headv.ident,VOCIDLEN)!=0) printf("ERROR: VOC: bad header");
  if(fseek(fp,headv.block1,SEEK_SET)) printf("ERROR: VOC: bad header");
  if(fread(&blockv,sizeof(struct VOCHEAD),1,fp)!=1) printf("ERROR: VOC: can't read block");
  if(blockv.type!=1) printf("ERROR: VOC: first block is not sound");
  datasize= ((blockv.sizeU)<<16)&0xFF0000L;
  datasize+=((blockv.sizeM)<<8)&0xFF00L;
  datasize+= (blockv.sizeL)&0xFFL;
  datasize -=2;
  /*check VOC header*/
  if(datasize>0x10000L) printf("ERROR: VOC: sample too long!");
  if(blockv.cmprs!=0) printf("ERROR: VOC: compression not supported.");
  smplrate= (1000000L)/(256-(((int)blockv.rate)&0xFF));
  /*read data*/
  data=(char  *)malloc(datasize);
  for(wsize=0;wsize<datasize;wsize+=sz)
  { sz = (datasize-wsize>MEMORYCACHE)? MEMORYCACHE:(datasize-wsize);
    if(fread((data+(wsize)),(size_t)sz,1,fp)!=1)    	printf("ERROR: VOC: can't read sound");
  }
  fclose(fp);
...
{
 char *data;
 Int32 datasize = 0, phys_size = 0;
 Int16 type = 0, headsize = 0;
 UInt16 speed = 0;
 headsize = sizeof(Int16)+sizeof(Int16)+sizeof(Int32);
 if(size < headsize)
{
  printf("WARNING: Sound %s: lump has no header. Skipping.", file);
  return;
}
 ...
 if (type!=3)
  printf("WARNING: Sound %s: weird type %d. Extracting anyway.",
   lump_name (name), type);

  phys_size = size - headsize;
  if (datasize > phys_size)
  {
    printf("WARNING: Sound %s: declared sample size %lu greater than lump size %lu ;",
     lump_name (name), (unsigned long) datasize, (unsigned long) phys_size);
    printf("WARNING: Sound %s: truncating to lump size.", lump_name (name));
    datasize = phys_size;
  }
  ...
  else if (datasize < phys_size)
  {
    ...
    {
      printf("WARNING: Sound %s: lump size %lu greater than declared sample size %lu ;",
	lump_name (name), (unsigned long) datasize, (unsigned long) phys_size);
      printf("WARNING: Sound %s: truncating to declared sample size.",
	  lump_name (name));
    }
  }
...
{
  case SNDWAV:
   SNDsaveWave(file,data,datasize,speed);
   break;
  case SNDAU:
   SNDsaveAu(file,data,datasize,speed);
   break;
  case SNDVOC:
   SNDsaveVoc(file,data,datasize,speed);
   break;
  default: printf("BUG: sndsv");
}
...
{
/*
 d = 0, s = 0, soundsize = 0, speed = 0;
 long rate = 0;
*/
 Int32  size = 0, datasize = 0, rate = 0;
 char  *data = NULL;
 long wadrate = 0;
 ...
 switch(Sound)
{
  case SNDWAV:
   ...
   break;
  case SNDAU:
   data = SNDloadAuFile(file,&datasize,&rate);
   break;
  case SNDVOC:
   data = SNDloadVocFile(file,&datasize,&rate);
   break;
  default: printf("BUG: sndcw");
}
/*
 ...
 if( speed > 11025 )
{
  printf("WARNING: shrinking %d to 11025 sample/s", speed);
...
{
  printf("WARNING: expanding %d to 11025 sample/s", speed);
  data = (char  *)realloc(data,soundsize);
...
{
  case RP_REJECT:
   if (rate != 11025) printf("%s: sample rate not 11025 Hz", fname(file));
   break;

  case RP_FORCE:
   if (rate > 11025)
{
    printf("%s: resampling down from %d Hz to 11025 Hz", fname (file), rate);
    wadrate = 11025;
    double ratio = 11025.0 / rate;
    long s = 0;
    datasize = (Int32) (ratio * datasize) + 1;
    for (s = 0; s < datasize; s++) data[s] = data[(size_t) (s / ratio + 0.5)];
    data = (char *) realloc(data, datasize);
}
   else if (rate < 11025)
{
    printf("%s: resampling up from %d Hz to 11025 Hz", fname (file), rate);
    wadrate = 11025;
    double ratio = 11025.0 / rate;
    long s = 0;
    datasize = (Int32) (ratio * datasize) + 1;
    data = (char *) realloc(data, datasize);
    for (s = datasize - 1; s >= 0; s--) data[s] = data[(size_t) (s / ratio + 0.5)];
}
   break;

  case RP_WARN:
   if (rate != 11025)
    printf("%s: sample rate != 11025 Hz, won't work on Doom < 1.4", fname (file));
   break;

  case RP_ACCEPT:
   break;

  default: printf("SNDcopyInWAD: rate_policy %d", (int) rate_policy);
}
...
{ FILE *fp;
  ...
  if(size<headsize)	printf("ERROR: WAV: wrong size");
  type     = peek_i16_le (buffer);
  datasize = peek_i16_le (buffer + 2);
  data= buffer+(sizeof(Int16)+sizeof(Int16));
  if(type!=0)		printf("BUG: SNDsavePCcound: not a PC sound");
  if(size<datasize+headsize)printf("ERROR: WAV: wrong size");
...
{ struct TXTFILE *Txt;
  ...
  { s=TXTreadShort(Txt);
    if((s<0)||(s>255))printf("ERROR: SND: number out of bounds [0-255]");
