...
{ char  *data;
  ...
  */
  printf("PHASE: Copying entries from Wad (Please wait).\n");
  data = (char  *)malloc(MEMORYCACHE);
  for(n=0;n<(rwad->ntry);n++)
  {
    if((n&0x7F)==0) printf("PHASE: .");
...
{ char  *data;
  ...
  */
  ...
  WADRseek(ewad,0);
  esize=ewad->maxpos;
  printf("PHASE: Inserting Wad file into Wad.\n");
  data = (char  *)malloc(MEMORYCACHE);
  ...
  { sz=(esize-wsize>MEMORYCACHE)? MEMORYCACHE:esize-wsize;
    ...
    if((wsize&0xF000)==0) printf("PHASE: .");
  }
  free(data);
  for(n=0;n<(rwad->ntry);n++)
  { /*detect external entries */
...
{ struct WADINFO rwad;
  ...
  FILE *fp;
  Bool Fail;
  printf("PHASE: Attempting to restore WAD %s\n",wadres);
  /*open DOOM.WAD*/
  rwad.ok=0;
  ...
  if(Fail != FALSE) printf("ERROR: Not a modified WAD");
  printf("PHASE: Restoration infos seem correct.\n");
  ...
  { /*extract the PWAD*/
    ...
    if(fp!=NULL)
    { fclose(fp);
      printf("INFO: File %s already exist: internal WAD discarded.\n",ewadfile);
    }
    else
    { printf("PHASE: Restoring internal WAD %s\n",ewadfile);
	...
	{ sz=(size-wsize>MEMORYCACHE)? MEMORYCACHE : size-wsize;
	  ...
	  { printf("WARNING: Can't write %d", ewadstart);break;
	  }
	}
	free(data);
	fclose(fp);
      ...
      printf("WARNING: Can't open %s",ewadfile);
    }
  }
  ...
  printf("Restoration of %s should be successful.\n",wadres);
}
/**************** End WAD restoration **********************/
...
{ static char name[8];
  ...
  printf("PHASE: Writing new WAD directory\n");
...
void PSTmergeWAD(const char *doomwad,const char *wadin,NTRYB select)
{
  ...
  printf("PHASE: Attempting to merge IWAD %s and PWAD %s\n",doomwad,wadin);
...
void ADDappendSpriteFloor(const char *doomwad, const char *wadres,NTRYB select)
{
  ...
  struct WADDIR  *NewDir;Int32 NewNtry;
  printf("PHASE: Appending ");
  if(select&BSPRITE) printf("PHASE: Sprites ");
  if(select&BFLAT)   printf("PHASE: Flats ");
  printf("PHASE: from IWAD %s to PWAD %s\n",doomwad,wadres);
    ...
    *wadext,NTRYB select)
{
  ...
  printf("PHASE: Merging PWAD %s into PWAD %s\n",wadext,wadres);
  /* get iwad directory, and identify */
  iwad.ok=0;
  WADRopenR(&iwad,doomwad);
  /*find PNAMES*/
  ...
  { /*add TEXTURE1 from rwad*/
   printf("PHASE:   With TEXTURE1 from %s\n",wadres);
   PatchSz=0;Patch=NULL;
   pnm=WADRfindEntry(&rwad,"PNAMES");
   if(pnm>=0)
   { printf("PHASE:   Declaring Patches from %s\n",wadres);
     riden[pnm]=EVOID;
   ...
   TXUreadTEXTURE(Entry,EntrySz,Patch,PatchSz,TRUE);
    ...
    /*TEXTURE1 from ewad*/
    printf("PHASE:   And TEXTURE1 from %s\n",wadext);
    PatchSz=0;Patch=NULL;
    ...
    *wadres,NTRYB select)
{
  ...
  printf("PHASE: Copying ");
  if(select&BSPRITE) printf("PHASE: Sprites ");
  if(select&BFLAT)   printf("PHASE: Flats ");
  printf("PHASE: from IWAD %s and PWAD %s\n",doomwad,wadres);
  printf("PHASE: Into PWAD %s\n",wadout);
  /* get iwad directory, and identify */
  iwad.ok=0;
  WADRopenR(&iwad,doomwad);
  /*find PNAMES*/
  ...
  free(Pnam);
  /*where to put pwad? at pwadstart*/
  if((iwad.maxpos|pwad.maxpos)&EXTERNAL )printf("BUG: AddExt");
  /* merge the two directories */
  NewDir=LISmergeDir(&NewNtry,TRUE,TRUE,select,&iwad,iiden,EXTERNAL,&pwad,piden,0);
  /* create a new PWAD*/
  rwad.ok=0;
  ...
  WADRwriteDir(&rwad, 1);
  WADRclose(&rwad);
  printf("Addition of Sprites and Floors is complete\n");
}
