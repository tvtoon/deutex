...
{ 
  if(TXTok!=TRUE) printf("BUG: TxtClo");
  fclose(TXT->fp);
  free(TXT);
}
...
{ struct TXTFILE *TXT;
  ...
  if(TXT->fp==NULL)  printf("ERROR: Could not open file %s for reading",file);
  return TXT;
}
...
{  Int16 c=0,val=0,n=0;
   ...
   { if(TXTgetc(TXT,&c,&val)!=TRUE)return FALSE;
     ...
     if(val & valid) break;
     printf("ERROR: Line %d: Illegal char '%c'",TXT->Lines,c);
   }
   name[0]=(char)c;
   for(n=1; n<256; n++ )
   { if(TXTgetc(TXT,&c,&val)!=TRUE) break;  
     ...
     { TXTungetc(TXT);break;}
     if(!(val&valid))
        printf("ERROR: Line %d: Illegal char '%c' in word",TXT->Lines,c);
...
{ if(TXTok!=TRUE) printf("BUG: TxtRid");
  ...
  if(TXTread(TXT,name,NAME|NUMBER)!=TRUE)      	printf("ERROR: Line %d: Expecting identifier or 'END:'",TXT->Lines);
  Normalise(name,name);
  return TRUE;
}
...
{ Int16 c=0,val=0;
  ...
  { printf("ERROR: invalid optional name");
  }
}
/*
 ...
 static char name[9];
  ...
  { if(TXTgetc(TXT,&c,&val)!=TRUE)return INVALIDINT;
   ...
   { if(TXTgetc(TXT,&c,&val)!=TRUE) break;
     ...
     if(val&SPACE)break;
     if(!(val&NUMBER))
	printf("ERROR: Line %d: Illegal char '%c' in number",TXT->Lines,c);
     if(n<8) name[n]=(char)c;
   }
   if(n<8)name[n]='\0';
   name[8]='\0';
...
{ Int16 c=0,val=0;
  ...
  if(TXT->SectionEnd>TXT->SectionStart) printf("BUG: TxtBdS");
...
{
  if(TXTok!=TRUE) printf("BUG: TxtSks");
  /*seek begin of file*/
  TXT->SectionStart=0;
  TXT->SectionEnd  =0;
...
{ if(TXTok!=TRUE) printf("BUG: TxtTxd");
  if(TXTskipComment(TXT)==FALSE) return FALSE; /*End*/
  if(TXTread(TXT,name,NAME|NUMBER)!=TRUE)	printf("ERROR: Line %d: Expecting identifier",TXT->Lines);
  Normalise(name,name);
...
{ if(TXTok!=TRUE) printf("BUG: TxtRpd");
  if(TXTskipComment(TXT)==FALSE) return FALSE;
  if(TXTcheckStartPatch(TXT)!=TRUE) return FALSE; /*not a patch line*/
  if(TXTread(TXT,name,NAME|NUMBER)!=TRUE)
	   printf("ERROR: Line %d: Expecting identifier",TXT->Lines);
  Normalise(name,name);
  ...
  *y=yy;
  ...
  { if(TXTgetc(TXT,&c,&val)!=TRUE)break;
    ...
    if(val & SPACE)          /*eat space*/
       { continue;}
    if(comment==FALSE)
       { printf("ERROR: Line %d: bad entry format",TXT->Lines);}
  }
  return TRUE;
}
...
{ struct TXTFILE *TXT;
  ...
  { fclose(TXT->fp);
    ...
    printf("WARNING: Appending to file %s",file);
  }
  if(TXT->fp==NULL)  printf("ERROR: Could not write file %s",file);
  return TXT;
}
...
{ 
  if (TXT == &TXTdummy)
     return;
  if(TXTok!=TRUE) printf("BUG: TxtAdS");
  fprintf(TXT->fp,"[%.8s]\n",def);
}
...
{ 
  if (TXT == &TXTdummy)
     return;
  if(TXTok!=TRUE) printf("BUG: TxtAdE");
...
{ 
  if (TXT == &TXTdummy)
     return;
   if(TXTok!=TRUE) printf("BUG: TxtAdC");
  fprintf(TXT->fp,"# %.256s\n",text);
}
