...
{ int res=PICNONE;
  ...
  else if(MakeFileName(file,DataDir,Dir,"",filenam,"gif")==TRUE)
  {
    ...
    if (! gif_warning)
    {
      ...
      printf("WARNING: Switch to PPM or BMP.");
      gif_warning = 1;
    }
  }
  ...
  if(res!=PICNONE)
    *size = PICsaveInWAD(rwad,file,Type,OfsX,OfsY,res);
    ...
    printf("WARNING: Could not find file %s, .ppm, .bmp or .gif",file);
  return res;
}
...
{  /*
   ...
   */
   ...
   printf("INFO: Translating %s into a %cWAD %s\n",texin,(type==IWAD)?'I':'P',PWADname);
   ...
   */
   ...
   {  if(TXTseekSection(TXT,"LEVELS"))
	...
	{ p=IDENTlevel(name);
	  if(p<0) printf("ERROR: Illegal level name %s", lump_name (name));
	  if(MakeFileName(file,DataDir,"LEVELS","",filenam,"WAD")!=TRUE)
		printf("ERROR: Can't find Level WAD %s",file);
	  printf("DETAIL: Reading level WAD file %s\n",file);
...
/*should read playpal file if exist*/
   if(select&(BGRAPHIC|BSPRITE|BPATCH|BFLAT))
   {
     ...
     if(entry<0) printf("ERROR: Can't find PLAYPAL in main WAD");
	...
	{  if(Repeat!=TRUE)
	   { WADRalign4(&rwad);     /*align entry on Int32 word*/
		 ...
		 if(CMPOcopyFromWAD(&size,&rwad,DataDir,"LUMPS",name,filenam)
		     !=TRUE)
		   printf("ERROR: Can't find Lump or picture file %s.",file);
	     }
	   }
   ...
   */
   ...
   { entry=WADRfindEntry(&iwad,"PNAMES");
     if(entry<0) printf("ERROR: Can't find PNAMES in main WAD");
   ...
   {  if(TXTseekSection(TXT,"TEXTURE1"))
	...
	{ if(MakeFileName(file,DataDir,"TEXTURES","",name,"TXT")==TRUE)
	  ...
	  { printf("DETAIL: Reading texture WAD %s\n",file);
	    ...
	    printf("ERROR: Can't find texture list %s",file);
	}
	/*write texture*/
	if(FoundOne==TRUE)
	{ WADRalign4(&rwad);     /*align entry on Int32 word*/
   ...
   */
   ...
   {  if(TXTseekSection(TXT,"TEXTURE2"))
	...
	else printf("WARNING: Can't find TEXTURE2 in main WAD");
	FoundOne=FALSE;
	...
	{ if(Repeat!=TRUE)
	  { WADRalign4(&rwad);     /*align entry on Int32 word*/
	    ...
	    { size=SNDcopyPCSoundInWAD(&rwad,file);
	      printf("DETAIL: Read PC Sound as file %s\n",file);
	    }
	    else
	    { if(MakeFileName(file,DataDir,"SOUNDS","",filenam,"WAV")==TRUE)
	      ...
	      { size=SNDcopyInWAD(&rwad,file,SNDVOC);
		    ...
		    filenam)!=TRUE)
		printf("ERROR: Can't find Sound %s, AU or WAV or WAD",file);
	      printf("DETAIL: Read Sound in file %s\n",file);
	    }
	  }
	  WADRdirAddEntry(&rwad,start,size,name);
	}
   ...
   {  start=size=0;
	...
	{ if(Repeat!=TRUE)
	  { WADRalign4(&rwad);     /*align entry on Int32 word*/
	    ...
	    { size=WADRwriteLump(&rwad,file);
	      printf("DETAIL: Read Music as MUS file %s\n",file);
	    }
	    else if(CMPOcopyFromWAD(&size,&rwad,DataDir,"MUSICS",name,
		  filenam)!=TRUE)
	      printf("ERROR: Can't find Music %s",file);
	  }
	  WADRdirAddEntry(&rwad,start,size,name);
	}
      }
   ...
   */
   ...
   {  FoundOne=FALSE;
      ...
      */
      ...
      { printf("PHASE: Making Wall Patches\n");
	while(TXTentryParse(name,filenam,&X,&Y,&Repeat,TXT,TRUE)==TRUE)
	{ if((Repeat!=TRUE)||(FoundOne!=TRUE))
	  { WADRalign4(&rwad);     /*align entry on Int32 word*/
	    start=WADRposition(&rwad);
	...
	{ printf("Reusing DOOM entry %s as patch\n", lump_name (name));
	}
...
{  FILE *out;Int16 n;
   ...
   for(n=0;n<HowMuchJunk;n++)
      if(fwrite(Junk,1,64,out)<64)  printf("WARNING: can't insert my junk!");
   fclose(out);
