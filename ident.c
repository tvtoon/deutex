...
{
  int n;
  for (n = 1; n < (int)( sizeof Part / sizeof *Part ); n++)
  {
...
int IDENTgraphic(struct WADINFO *info,Int16 n)
{
  ...
  buf = malloc(size);
  WADRseek(info,start);
  WADRreadBytes (info, (char *)buf, size);
...
{ Int16 n;
  ...
  if(n>=0)   /*found it?*/
	...
	if (debug_ident != NULL
	  ...
	  printf("INFO: Ident: %-8s as %-8.32s by %.32s\n",
...
{ Int16 p_end,p_start;
  ...
  */
  ...
  {
    printf("BUG: Bad tl %d", (int) texture_lump);
  }
	...
	else if(Check==TRUE)
	{
	  ...
	  else
	  {
	    if (is_snea > 0 && is_picture > 0 && is_snea == is_picture)
	      printf("WARNING: Ambiguous type for %s (picture or snea ?)",
...
{ Int16 n,l;
  ...
  for (n = 0; n < info->ntry; n++)
  {
	...
	in_level = 0;
	{
	  ...
	  for (i = 0; i <= max_lumps; i++)
	    if (! lump_present[i] && Part[i].mandatory)
	      printf("WARNING: Level %s: no %s lump", level_name, Part[i].name); 
#endif
	  for (i = n0; i <= n; i++)
	...
	if (have_lump[l])
	{
	  printf("WARNING: Level %s: duplicate %s lump",
	...
	{
	  if (! wrong_order)
	    printf("WARNING: Level %s: lumps in the wrong order (%s)",
	...
	if (level_lump == 0)
	{
	  ...
	  for (n = 0; n <= max_lumps; n++)
	    if (! have_lump[n] && IDENTlevelPartMandatory (n))
	      printf("WARNING: Level %s: no %s lump", IDENTlevelPartName (n)); 
	}
    }
  ...
  if (in_level)
    printf("BUG: Reached EOD while in level");
}
...
{ Int16 n;
  Bool Doom=FALSE;
  ENTRY  *ids;
  printf("PHASE: IWAD entry identification...");
  if (debug_ident != NULL)
    printf("PHASE: \n");
  if(info->ok!=TRUE)printf("BUG: IdnOeI");
...
/*
  switch(check)
  { case 1: case 2: break;
	 default: printf("ERROR: Please register your game.");
  }
*/
...
{ Int16 n;
  ENTRY  *ids;
  printf("PHASE: PWAD entry identification...");
  if (debug_ident != NULL)
    printf("PHASE: \n");
  if(info->ok!=TRUE)printf("BUG: IdnOeP");
  ids=(ENTRY  *)malloc((info->ntry)*sizeof(ENTRY));
  /*
  ** identify for PWAD
  */
